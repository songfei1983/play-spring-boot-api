...
**Docker Compose 配置**
- 添加了完整的 `docker-compose.yml` 文件，支持 API 和前端服务的容器化部署
- 配置了多服务编排，包括后端 API、前端应用和反向代理
- 支持通过环境变量灵活配置服务参数
- 添加了服务间网络通信和端口映射配置

**多阶段构建优化**
- 为前端应用创建了优化的 `Dockerfile`，使用 Nginx 作为 Web 服务器
- 实现了构建阶段和运行阶段的分离，减小镜像体积
- 配置了 Nginx 反向代理，统一处理前后端请求路由
- 添加了健康检查机制，确保服务可用性

**容器化部署支持**
- 创建了详细的 `DOCKER_SETUP.md` 部署指南
- 提供了完整的容器化部署流程和配置说明
- 支持开发环境和生产环境的不同配置
- 添加了容器日志管理和监控配置
...
**统一命令管理**
- 添加了 `justfile`，提供统一的项目命令管理界面
- 集成了开发、构建、测试、Docker 操作等完整工作流命令
- 简化了复杂的 Maven 和 Docker 命令操作
- 提供了一键式的项目启动和部署命令

**开发工作流优化**
- 包含代码格式化、代码检查、安全扫描等质量保证命令
- 提供数据库控制台、健康检查、URL 打开等实用功能
- 支持前后端分离的开发和部署流程
- 集成了测试覆盖率报告生成和查看命令

**配置文件管理**
- 更新了 `.gitignore`，添加 Docker 和 Just 相关文件的忽略规则
- 添加了 Nginx 配置文件，支持前端路由和 API 代理
- 优化了开发环境的配置管理
